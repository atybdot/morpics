...
const inputVariants = cva(
  ...
  `,
  {
    variants: {
      variant: {
        lg: "py-1 min-h-10 px-4 text-sm [&~[data-slot=combobox-icon]]:end-2.5 [&~[data-slot=combobox-clear]]:end-7",
        md: "py-1 min-h-9 px-3 text-sm [&~[data-slot=combobox-icon]]:end-2 [&~[data-slot=combobox-clear]]:end-6",
        sm: "py-0.5 min-h-8 px-2.5 text-xs [&~[data-slot=combobox-icon]]:end-1.75 [&~[data-slot=combobox-clear]]:end-5.75",
      },
    },
    defaultVariants: {
      variant: "md",
  ...
  return (
    <ComboboxPrimitive.Popup
      ...
      className={cn(
        ...
        "border border-border bg-popover text-popover-foreground shadow-md shadow-black/5",
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        ...
        "text-foreground relative select-none items-center gap-2 ps-7 pe-2 py-1.5 text-sm outline-hidden transition-colors data-disabled:pointer-events-none data-disabled:opacity-50",
        "[&_svg]:pointer-events-none [&_svg:not([role=img]):not([class*=text-])]:opacity-60 [&_svg:not([class*=size-])]:size-4 [&_svg]:shrink-0",
        "data-[highlighted]:relative data-[highlighted]:z-0 data-[highlighted]:text-foreground data-[highlighted]:before:absolute data-[highlighted]:before:inset-x-1 data-[highlighted]:before:inset-y-0 data-[highlighted]:before:z-[-1] data-[highlighted]:before data-[highlighted]:before:bg-accent",
        className,
      )}
      {...props}
    />
...
function ComboboxClear({
  ...
  return (
    <ComboboxPrimitive.Clear
      ...
      className={cn(
        "absolute cursor-pointer end-6 top-1/2 -translate-y-1/2 opacity-70 ring-offset-background",
      ...
      {...props}
    >
      {children ? children : <PiX className="size-3.5 opacity-100" />}
    </ComboboxPrimitive.Clear>
  );
}
...
function ComboboxIcon({
  ...
  return (
    <ComboboxPrimitive.Icon
      ...
      {...props}
    >
      {children ? children : <PiCaretDown className="size-4 opacity-100" />}
    </ComboboxPrimitive.Icon>
  );
}
...
function ComboboxChipRemove({
  ...
  return (
    <ComboboxPrimitive.ChipRemove
      ...
      className={cn(
        "cursor-pointer ms-1 [&_svg]:opacity-60 hover:[&_svg]:opacity-100 h-full",
        className,
      )}
      {...props}
    >
      {children ? children : <PiX className="size-3.5" />}
    </ComboboxPrimitive.ChipRemove>
  );
}
