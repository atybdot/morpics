import { pgTable, unique, text, boolean, timestamp, foreignKey, integer, uuid, primaryKey, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const imageStatusEnum = pgEnum("image_status_enum", ['pending', 'success', 'failed', 'orphan'])
export const mimeEnum = pgEnum("mime_enum", ['image/png', 'image/webp', 'image/avif', 'image/jpeg', 'image/tiff', 'image/jp2'])
...
export const transformationMetadata = pgTable("transformation_metadata", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	transformationId: uuid("transformation_id").notNull(),
	fileSize: integer("file_size").notNull(),
	width: integer(),
...
export const imageTags = pgTable("image_tags", {
	...
	primaryKey({ columns: [table.imageId, table.tagId], name: "image_tags_image_id_tag_id_pk"}),
]);
